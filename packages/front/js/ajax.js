...
function getVoucherInfo() {
	...
	loadXMLDoc("index.php?option=com_simplecaddy&no_html=1&action=vouchers&task=getvoucherinfo&voucher="+voucher, setVoucherInfo);
}

function getShipCost(region){
	document.checkout.scodrbtn.disabled="true";
	document.getElementById("ajaxloader").style.display="block";
	loadXMLDoc("index.php?option=com_simplecaddy&no_html=1&action=shipping&task=loadshipcost&shipRegion="+region, updateShipCost);
}
